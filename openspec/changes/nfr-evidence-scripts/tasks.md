...
- [ ] 4.3 真 `.pkl` 交付后重跑性能组，追加真模型证据 md；回填飞书《产品需求文档》第七章性能/安全实测结果（9/23 截止）
      —— 前半已完成：`perf-real-2026-09-20.md`（xgb_v1，p95=21.6ms 达标）进仓。**后半实况（第三次订正，选评审方案 B）**：内容已以**附件块**挂入第七章末（token S2ybbLgsTo0fMbxczn3cyzbWnrb，3137 B），正文段落仍为模板（提示框+占位句）、变更日志仍 v0.4 —— 正文粘贴与 v0.5 变更日志待 09-23 前补；误记史：①信口头「已贴」②把附件预览页误认正文截图③同②。补完后在 nfr 归档提交勾回并附 v0.5 变更日志行号
