...
- [x] 4.3 真 `.pkl` 交付后重跑性能组，追加真模型证据 md；回填飞书《产品需求文档》第七章性能/安全实测结果（9/23 截止）
      —— 2026-09-20 真模型重跑完成：`perf-real-2026-09-20.md`（xgb_v1，p95=21.6ms 达标）进仓；2026-09-21 飞书《产品需求文档》第七章已回填真模型实测版（正文可见 p95 21.6ms / 证据出处 / 复现命令引用，**截图核验**——09-21 曾误记「已回填」并订正过一次，本次为核验后勾选）
