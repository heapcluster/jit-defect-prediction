...
- [x] 4.3 真 `.pkl` 交付后重跑性能组，追加真模型证据 md；回填飞书《产品需求文档》第七章性能/安全实测结果（9/23 截止）
      —— 2026-09-20 真模型重跑完成：`perf-real-2026-09-20.md`（xgb_v1，p95=21.6ms 达标）进仓；2026-09-21 飞书《产品需求文档》第七章**正文**已粘贴真模型实测版（占位句与撰写人提示块已删、附件块已移除），截图核验为无框正文（p95 21.6ms / 证据出处 / 复现命令引用可见）。过程教训入证据行：此前两次误记（口头「已贴」、附件预览误认正文），本次以「无框正文 + 可检索」为核验标准
