...
> 起草 2026-09-16 ｜ 起草人 蒋励 ｜ 关联契约：`docs/contracts/feature-columns.md`（v1.0 已冻结）、`data-fields.md`（表三 `prediction`）、`api-format.md`
...
- **不改** 三条契约的字段名、列名、接口路径 —— 契约已冻结（契约二 v1.0），只在实现层遵守；SHAP 定稿只回填契约三「待确认」一节，不动正文格式
