...
> Sprint 1 ｜ 对应飞书《工作周报》「模型线训练与交付」；看板同名列（迭代 Sprint 1）
> 起草 2026-09-16 ｜ 起草人 蒋励 ｜ 关联契约：`docs/contracts/feature-columns.md`（契约二，冻结日期 2026-09-18）、`data-fields.md`（表三 `prediction`）、`api-format.md`
>
> ⚠️ **契约基线的判定方式：以各契约文件头部的「版本 + 冻结日期」为准，不引用 PR 编号或合并时机。**
>
> 上一版写的是「以 PR #5 与 PR #6 合并后的 `main` 为准」—— 这个条件**不可判定**：PR #6 的 §4 当时还没对齐，而且合并过程中版本号会被重排（#6 已把契约一重排为 `1.0 → 1.4`、契约二重排为 `1.0 → 1.2`）。**换成与合并时机无关的写法**：本 change 只声明「锚定契约的**冻结版本**」，具体版本号以文件头为准；合并导致版本号变化时，只需同步本文件的版本号引用，**规格内容不变**。
>
> 这条依赖同时登记在 `design.md` 的 Risks 一节。
...
- **新增「特征抽取」能力登记**：把已实现的 `03_extract_features.py` 与既有产物（`reports/feature_stats.md`）登记为 spec capability `feature-extraction`，口径对齐契约二（冻结版，冻结日期 2026-09-18）（`rexp` 衰减公式、六项 DECIMAL、`ln` 0 点不连续的 v1 已知局限、`nuc` 按文件计数求和）。**不是新代码，是补规格欠账**
...
- **新增「模型交付」能力**：`.pkl` 导出至 `data_model/models/`（已被 `.gitignore` 排除，不入库），随包交付特征顺序清单（按契约二第三节列序）；`model_name` 命名规则 `<算法>_v<序号>`（如 `xgb_v1`，与契约一表三示例一致），迭代换名不覆盖；SHAP 作为解释算法，SHAP 实时计算需过后端非功能要求验证（95% 请求 < 500ms）
...
- **不改** 三条契约的字段名、列名、接口路径 —— 契约已冻结（契约二（冻结版，冻结日期 2026-09-18）），只在实现层遵守；**契约三「待确认」一节已由 PR #6 结清**（`explanation` 的具体算法已定为 SHAP），故本 change **不再有「SHAP 定稿后回填契约」这一动作**，也不动契约正文格式
...
- `feature-extraction`: 从样本集算 Kamei 14 项特征，列名与取值口径照契约二（冻结版，冻结日期 2026-09-18），历史类特征用完整历史，产出特征表与 `feature_stats` 报告（登记已实现内容）
...
| 消费的契约 | `feature-columns.md`（特征列序与口径）、`data-fields.md` 表三 `prediction`（结果文件字段）、`api-format.md`（`explanation` 字段取值 SHAP）—— 只读不改，**本 change 无任何契约回填动作** |
