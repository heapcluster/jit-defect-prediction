...
- [ ] 2.2 特征表 `feature_version` 列值确认为 `v1`（契约二 v1.0），列名与契约二第三节逐字比对 —— 验证：比对输出「缺列 0 / 多列 0 / 改名 0」
...
- [ ] 4.1 模型导出 `.pkl` 至 `data_model/models/`（不入库），随包产出特征顺序清单（契约二第三节 v1.0 列序）—— 验证：`git status --short` 不出现 models/ 项；后端按清单组装向量推理一条提交，结果与训练侧离线预测一致
...
- [ ] 4.4 SHAP 定稿后回填契约三「待确认」项（`explanation` 算法 = SHAP）；实测 SHAP 单条推理耗时并记录，评估是否满足 95% 请求 < 500ms —— 验证：实测数据进报告；若不达标，契约三变更提案已提（不静默降级）
