...
- [ ] 3.2 `prediction` 表加 `(commit_hash, model_name)` 唯一索引（design D7，契约一允许的后端自属冗余索引）
...
- [ ] 4.8 `POST /api/predict`：`commit_hash` 必填且 40 位十六进制（违例 40001）；不在 `commit` 表 40400 `commit not found`；无特征行 40400 `features not found for this commit`；实时推理（MUST NOT 查 `prediction` 表代替）；结果按 D7 幂等落库
