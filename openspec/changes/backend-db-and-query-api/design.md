...
聊天裁定内部有两句口径：§1 幂等 bullet「不写库、不改状态」与 §3「**A 与 C 写同一张 prediction 表，靠 model_name 区分来源**」。契约三 v1.2（裁定指定的落点、已冻结）取后者：「库里只保留最新一行（覆盖或先删后插），不得累积重复行」。2026-09-17 用户复核确认：predict 需要写 `prediction` 表。实现为 update-or-insert：
