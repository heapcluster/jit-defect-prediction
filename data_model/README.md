...
4. 只在本目录内写代码：**不写接口、不写页面、不建库表**。唯一例外是方案 C —— 离线预测结果要写进 `prediction` 表，那是**往后端建好的表里插数据**，不是建表；表结构仍以 `docs/contracts/data-fields.md` 为准，本线不改、也不跑建表脚本。
...
| **C. 离线预测结果入库** | ✅ **采用** | 数据线批量预测写 `prediction` 表，供风险列表 / 提交详情 / 趋势看板三个查询接口读 |
