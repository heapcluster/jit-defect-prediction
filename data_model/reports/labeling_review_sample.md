...
> **一处渲染说明**：B 组第 28 条（`c1a2ff25c127`）的提交信息里含 2 个孤立 CR（`\r`）——
> 这是仓库原始提交信息的原样内容，`main` 上即存在，本次复核**未增删**：`origin/main` 与本次提交
> 均为 `CR=2`、`CRLF=0`，上下文（`Remove activemq-camel` 之后）相同，仅字节偏移因新增正文而后移。
> 部分渲染器会在此处断行，属**显示现象，不是数据错误**。
> 不做清洗的理由：清洗它等于改动被抽样的原始数据本身，A/B 两组的可信度随之失效。
