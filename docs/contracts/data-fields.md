...
> 状态：**已冻结** ｜ 版本 1.4 ｜ 定稿人 蒋励 + 吕建江 ｜ 冻结日期 2026-09-18 ｜ 冻结人 蒋励、吕建江
> 1.2 版为**打标方法说明的改口**：`szz` 的括注由「（PySZZ）」改为「标准行级 SZZ，自研实现」—— PySZZ 不在 PyPI，实测结论见表二脚注。1.4 版为 **ER 图中 `ns`/`nd`/`nf` 由 INT 改为 DECIMAL**。**取值枚举、列名、类型、索引均未改动**（1.1 版新增的「建表归属」一节见变更日志）。
...
erDiagram
    ...
    commit_feature {
        ...
        DECIMAL ns "特征：改动的子系统数"
        DECIMAL nd "特征：改动的目录数"
        DECIMAL nf "特征：改动的文件数"
...
| 2026-09-18 | 1.4 | 蒋励 | **ER 图中 `ns`/`nd`/`nf` 由 INT 改为 DECIMAL**：契约二 v1.0 已把这六项改成 DECIMAL（取对数后必为小数），契约一 ER 图若仍是 INT，后端线照图建表会得到「数据线落小数、后端建 INT 列」。<br>**版本号重排说明（审阅意见）**：本条原编号 0.92、且在 1.1 之后，而文件头写 1.2 —— 头与日志对不上、顺序也乱。现按「日期 + 内容顺序」重排为 **1.0 → 1.1 → 1.2 → 1.3 → 1.4**，文件头同步为 1.4 |
