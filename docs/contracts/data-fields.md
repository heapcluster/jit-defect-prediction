...
| 2026-09-18 | 0.92 | 蒋励 | **ER 图中 `ns`/`nd`/`nf` 由 INT 改为 DECIMAL**：契约二 v1.0 已把这六项改成 DECIMAL（取对数后必为小数），契约一 ER 图若仍是 INT，后端线照图建表会得到「数据线落小数、后端建 INT 列」。为修同一处自相矛盾而动的同一文件，按审阅意见不算夹带。⚠️ 本条与 PR #6（契约一 1.2 冻结）改同一文件，**合并顺序须为先本 PR、后 #6**，#6 合并时保留本条改动 |
