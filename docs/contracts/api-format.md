...
> 状态：**已定稿，2026-09-23 正式宣布冻结**（2026-09-16 已提前结清全部待确认项）｜ 版本 1.0
...
| 鉴权 | 固定令牌，请求头 `X-API-Key`；未通过返回 401 | 对应课程「接口鉴权」要求，细则见下方 |
...
**本契约共三个接口，不再新增第四个** —— 关于「模型下拉的选项从哪来」，见第六节冻结结论。
...
> **本响应不返回「真实标签 / 模型判对了没有」。** 理由见第六节冻结结论第 1 条。
...
> `series` **只返回单个 `model_name` 的一组数据**，不做同图多模型对比；`model_name` 参数用于切换查看哪个模型，不是叠加。见第六节冻结结论第 1 条。
...
## 冻结结论（原「待确认」，2026-09-16 结清）

| 原待确认项 | 结论 |
|---|---|
| 三个接口是否够画三个页面（尤其趋势看板是否需要按作者 / 子系统分组） | **够，接口数量维持三个，不新增第四个**。三处定论：① **趋势看板不做按作者/子系统分组** —— MVP 固定为「风险随时间变化」单一维度，分组要同时改请求参数、响应结构与前端联动，课程未要求；② **不做同图多模型对比** —— `series` 保持单模型一组数据，`model_name` 只用于切换查看；③ **模型下拉的选项来源：MVP 由前端维护已知模型版本清单**（本期只有 2–3 个且极少变动），不为此新增 `GET /api/models`。若 Sprint 1 模型数量增长到需要动态获取，届时按契约变更流程新增接口 |
...
> 上述第一、三条涉及 `docs/pages.md` 第 6 节的同名待确认项，两处口径一致，改动时须同步改。**自 2026-09-16 起，本契约的任何改动均需走契约变更流程（提 OpenSpec change 并通知到人），不再接受口头调整。**
