...
> 状态：**进行中，第 ① 步已完成** ｜ 版本 0.92 ｜ 负责人 蒋励 ｜ 冻结时间 Sprint 0 末（9/25）
...
### 打标判据（2026-09-16 预跑探测）

修缺陷提交的判据（见 `openspec/changes/data-collection-and-szz-labeling/design.md` 决策 D4）：**缺陷编号 与 修复语义 双命中**。在固定版本上按 git 提交信息直接统计，得到判据的命中外沿：

| 判据 | 命中数 | 占 11,050 条非合并提交 |
|---|---|---|
| 只要求含 `AMQ-<数字>` 编号 | 6,041 | 54.7% |
| 编号 **且** 含 `fix`/`bug`/`patch` | 2,538 | 23.0% |
| 编号 **且** 含 `fix` 词形 | 1,865 | 16.9% |

**这组数字说明两件事**：① 用「只含编号」当判据会把 54.7% 的提交都拉进候选池，包含大量新功能与文档提交 —— 印证了必须加修复语义这一条；② 打标成功的提交数上限在 1,865 ~ 2,538 之间，**正样本比例不可能接近 50%**，与 JIT 研究的经验区间一致。
